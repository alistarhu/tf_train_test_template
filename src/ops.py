...
def conv2d_block(input, out_dim, k, s, is_train, name, bias=False):
    with tf.name_scope(name), tf.variable_scope(name):
        net = conv2d(input, out_dim, k, k, s, s, name='conv2d', bias=bias)
    ...
    with tf.name_scope(name), tf.variable_scope(name):
        ...
        if shortcut and stride == 1:
            ...
            if in_dim != output_dim:
                ins=conv_1x1(input, output_dim, name='ex_dim', bias=bias)
                net=ins+net
            else:
                net=input+net
